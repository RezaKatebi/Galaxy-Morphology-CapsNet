...
class CapsuleNet(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        if y is None:
            ...
            # ).index_select(dim=0, index=max_length_indices)
            y = Variable(torch.eye(NUM_CLASSES)).cuda(
            ).index_select(dim=0, index=max_length_indices)
...
if __name__ == "__main__":
    ...
    def on_forward(state):
        ...
        meter_loss.add(state['loss'].item()) #.data[0])
    ...
    def on_end_epoch(state):
        ...
        if state['epoch'] % 10 == 0:
            np.save('../results/ground_truth_{}_epoch'.format(state['epoch']), ground_truth)
            np.save('../results/reconstruction_{}_epoch'.format(state['epoch']), reconstruction)
    ...
    np.save("../results/train_accuracies", train_accs)
    np.save("../results/test_accuracies", test_accs)

    np.save("../results/train_losses", train_losses)
    np.save("../results/test_losses", test_losses)

    np.save("../results/confs", confs)
